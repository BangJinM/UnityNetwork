    ...
    public class KHttpDownloader : IDisposable
    {
        ...
        private void ThreadableResumeDownload(string url, Action<int, int> stepCallback, Action errorCallback, Action successCallback)
        {
            ...
            if (_useContinue && System.IO.File.Exists(TmpDownloadPath))
            {
                ...
                if (_isLog) Console.WriteLine("Resume.... from {0}", lStartPos);
            }
            else
            {
            ...
            try
            {
                ...
                if (_isLog) Console.WriteLine("Getting Response : {0}", url);

                //向服务器请求，获得服务器回应数据流 
                using (var response = request.GetResponse()) // TODO: Async Timeout
                {
                    TotalSize = (int)response.ContentLength;
                    if (_isLog) Console.WriteLine("Getted Response : {0}", url);
                    if (IsFinished)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            if (_isLog) Console.WriteLine("Start Stream: {0}", url);
                ...
                if (_isLog) Console.WriteLine("下载完成: {0}", url);
            ...
            {
                if (_isLog) Console.WriteLine("下载过程中出现错误:" + ex.ToString());
                ...
                {
                    if (_isLog) Console.WriteLine("删除临时下载文件出错:" + e.Message);
                }
        ...
        public void Dispose()
        {
            if (!FinishedFlag)
            {
                FinishedFlag = true;
                ErrorFlag = true;
                Console.WriteLine(string.Format("[HttpDownloader]Not finish but Dispose: {0}", Url));
            }
        }
    }
}